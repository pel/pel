...
class PelEntryTime extends PelEntryAscii {
    ...
    function getValue($type = self::UNIX_TIMESTAMP) {
        switch ($type) {
            ...
            default :
                throw new PelInvalidArgumentException ( 'Expected UNIX_TIMESTAMP (%d), ' . 'EXIF_STRING (%d), or ' .
                     'JULIAN_DAY_COUNT (%d) for $type, ' . 'got %d.', self::UNIX_TIMESTAMP, self::EXIF_STRING, self::JULIAN_DAY_COUNT, $type );
        }
    }
